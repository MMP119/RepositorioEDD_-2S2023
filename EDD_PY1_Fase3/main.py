...
class ProjectUpApp:
    ...
    def obtener(self):
        ...
        contraEncrip = ProjectManagerApp.encriptarContra(self, password)
        print(password)
        print(contraEncrip)
        ...
        elif tablaH.buscar(usuario, contraEncrip) and tablaH.buscarProjectManager(usuario, contraEncrip) == False:
            print('empleado')
            self.root.destroy()
            emp = Tk()
        ...
        elif tablaH.buscarProjectManager(usuario, contraEncrip) and tablaH.buscar(usuario, contraEncrip):
            print("Bienvenido")
            self.root.destroy()
            projec = Tk()
            app = ProjectManagerApp(projec, tablaH,usuario)
...
class ProjectManagerApp:
    ...
    def load_assets(self):
        ...
        self.buttonReporteGrafo = PhotoImage(file =self.relative_to_assets("ReporteGrafo.png"))
        self.buttonMerkle = PhotoImage(file =self.relative_to_assets("merkle.png"))
        self.buttonAnteSuce = PhotoImage(file =self.relative_to_assets("AnteSuce.png"))
        self.buttonBlockChain = PhotoImage(file =self.relative_to_assets("Blockchain.png"))
        self.buttonEWallet = PhotoImage(file =self.relative_to_assets("eWallet.png"))


    def create_widgets(self):
        self.canvas.create_image(350.0, 276.0, image=self.image_image_1)
        self.canvas.create_image(616.0, 55.0, image=self.image_image_2)
        self.canvas.create_image(351.0, 336.0, image=self.image_image_3)
        self.canvas.create_image(459.5, 55.0, image=self.entry_image_1)
        self.entry_1 = Text(
            bd=0,
            bg="#FFFFFF",
            fg="#000716",
        ...
        self.entry_1.place(x=382.0, y=40.0, width=155.0, height=28.0)
        ...
        self.ReporteGrafo = Button(
            image = self.buttonReporteGrafo,
            borderwidth = 0,
            highlightthickness = 0,
            command = lambda: self.imprimirGrafo(),
            relief = "flat"
        )
        self.ReporteGrafo.place(x=239.0, y=457.0, width=232.0, height=43.0)

        self.Merkle = Button(
            image = self.buttonMerkle,
            borderwidth = 0,
            highlightthickness = 0,
            command = lambda: print("Merkle"),
            relief = "flat"
        )
        self.Merkle.place(x=81.0, y=387.0, width=232.0, height=43.0)

        self.BlockChain = Button(
            image = self.buttonBlockChain,
            borderwidth = 0,
            highlightthickness = 0,
            command = lambda: print("BlockChain"),
            relief = "flat"
        )
        self.BlockChain.place(x=385.0, y=387.0, width=232.0, height=43.0)

        self.AnteSuce = Button(
            image=self.buttonAnteSuce,
            borderwidth=0,
            highlightthickness=0,
            command=lambda: self.abrirAntecesores(),
            relief="flat"
        )
        self.AnteSuce.place(x=385.0, y=328.0, width=232.0, height=43.0)

        self.eWallet = Button(
            image=self.buttonEWallet,
            borderwidth=0,
            highlightthickness=0,
            command=lambda: self.abrirEWallet(),
            relief="flat"
        )
        self.eWallet.place(x=81.0, y=328.0, width=232.0, height=43.0)
    ...
    def abrirArchivoCSV(self):
        try:
            ...
            for row in reader:
                ...
                contraEncrip = self.encriptarContra(password)
                self.tablaH.Insertar(id,nombre,contraEncrip,puesto)
    ...
    def abrirEWallet(self):
        try:
            abrirArch = filedialog.askopenfile(title = 'Abrir Archivo', filetypes = [('All Files', '*.*'),('JSON Files', '*.json')])
            with open(abrirArch.name,'r') as file:
                data = json.load(file)

            for i in range(len(data['eWallet'])):
                empleado = data['eWallet'][i]['empleado']
                id = data['eWallet'][i]['id']
                self.tablaH.asignarEWallet(empleado, id)

            messagebox.showinfo(message="Archivo Cargado Correctamente")
            #for clave, valor in tablaH.tabla.items():
                #print(clave, valor.codigo, valor.nombre, valor.password, valor.puesto, valor.ewallet)
    ...
    def abrirAntecesores(self):
        try:
            ...
            # Por cada tarea, agregamos el nodo al grafo y luego sus conexiones (antecesores)
            for tarea in data['Tareas']:
                codigo = tarea['codigo']
                grafo.insertarFila(codigo)  # Agregamos el nodo principal

                # Por cada antecesor, agregamos una conexión en el grafo
                for antecesor in tarea['antecesor']:
                    grafo.agregarNodo(codigo, antecesor['codigo'])
    ...
    def imprimirGrafo(self):
        grafo.Grafico()
        messagebox.showinfo(message="Grafo generado!")

    def encriptarContra(self, password):
        # Crear un objeto SHA256
        sha256 = hashlib.sha256()
        
        # Codificar la contraseña y pasarla al objeto SHA256
        sha256.update(password.encode('utf-8'))
        
        # Obtener el hash encriptado en formato hexadecimal
        password_encriptado = sha256.hexdigest()
    ...
    def abrir_ventana_principal(self):
        ...
        ventana_principal.geometry("820x320")
        ...
        tabla.column("#0", width=55, stretch=False)
        tabla.column("#1", width=120, stretch=False)
        tabla.column("#2", width=130, stretch=False)
        tabla.column("#3", width=400, stretch=False)
        tabla.column("#4", width=120, stretch=False)

        def AgregarTabla():
            tabla.delete(*tabla.get_children())
            for clave, valor in self.tablaH.tabla.items():
                print(f"Clave: {clave}, Valor: {valor.codigo}")
                tabla.insert("", "end", text=clave,values=(valor.codigo, valor.nombre, valor.password, valor.puesto))

        scrollbar_horizontal = ttk.Scrollbar(ventana_principal, orient="horizontal", command=tabla.xview)
        ...
        boton_cerrar_sesion.grid(row=2, column=0, pady=12)
...
class Empleado:
    ...
    def create_widgets(self):
        ...
        self.estadoTareas = ttk.Combobox(self.root,
            state="readonly",
            values=["Sin comenzar", "En proceso", "Terminado"])
        self.estadoTareas.place(x=320.0, y=150.0, width=100.0, height=33.0)

        self.estado = Label(self.root, text="Estado de la tarea:")
        self.estado.place(x=200.0, y=155.0)
